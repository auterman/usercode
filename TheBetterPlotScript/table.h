...
public:
    ...
    virtual ~TTable() {
        for (std::vector<TColumnBase*>::iterator it=table_.begin(); it!=table_.end(); ++it) delete *it;
        table_.clear();
    };

    ///Method to specify the columns in this table. To be called once per column
    template <class T>      void AddColumn(const std::string& header) {
        table_.push_back(new TColumn<T>(header));
    };
    ...
    template <class T>      void Set(T value) {
        table_.at(idx_++)->Add( new T(value) );
        if(idx_>=table_.size())idx_=0;
    };
    /*spezialize for char*/ void Set(const char* value) {
        table_.at(idx_++)->Add( new std::string(value));
        if(idx_>=table_.size())idx_=0;
    };
    ...
    template <class T>      void Set(unsigned idx,T value) {
        table_.at(idx)->Add( new T(value) );
        idx_=(idx==table_.size()?0:idx+1);
    };
    /*spezialize for char*/ void Set(unsigned idx,const char* value) {
        table_.at(idx)->Add( new std::string(value) );
        idx_=(idx==table_.size()?0:idx+1);
    };

    ///Three arguments: This method can be used to overwrite the contents of cell (x,y) with value.
    template <class T>      void Set(unsigned x, unsigned y, const T& value) {
        static_cast<TColumn<T> >(table_.at(x))->Set(y,value);
    };
    ...
    void SetStyle(const TableStyle s) {
        style_=s;
    };
    TableStyle GetStyle() const {
        return style_;
    };
    void SetMinumumWidth(unsigned w) {
        table_.at(idx_)->SetMinumumWidth(w);
    }
    void SetMinumumWidth(unsigned w,unsigned idx) {
        table_.at(idx)->SetMinumumWidth(w);
    }
    unsigned GetWidth() {
        return table_.at(idx_)->GetCurrentWidth();
    }
    unsigned GetWidth(unsigned idx) {
        return table_.at(idx)->GetCurrentWidth();
    }
    std::string hline(char c='-', char r='+') const;


    ///Method to set the precision of column i. To be used e.g for <double> columns. Default is 1.
    void SetPrecision(unsigned i, unsigned p) {
        table_[i]->SetPrecision(p);
    };

    ///Set all columns to one precision, default precision is 1
    void SetPrecision(unsigned p) {
        for (std::vector<TColumnBase*>::const_iterator it=table_.begin(); it!=table_.end(); ++it) (*it)->SetPrecision(p);
    };

    const std::vector<TColumnBase*> * GetTable() const {
        return &table_;
    };
