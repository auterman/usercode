...
public:
    ...
    virtual std::string Str(const unsigned i) {
        return "";
    };
    virtual unsigned    Width() {
        return 0;
    };
    virtual unsigned    GetCurrentWidth() {
        return 0;
    };
...
public:
    ...
    virtual void        Add(const void * f) {
        field_.push_back( static_cast<const T*>(f));
    };
    virtual const void* Get(unsigned idx) {
        return field_[idx];
    };
    virtual unsigned    Size() {
        return field_.size();
    };
    virtual void        SetMinumumWidth(unsigned p) {
        width_=p;
    };
    ...
    virtual unsigned    GetCurrentWidth() {
        return (width_>0?width_:Width());
    };

    void Set(unsigned i, T f) {
        *field_.at(i)=f;
    };
