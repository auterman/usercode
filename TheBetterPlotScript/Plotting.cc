...
=======
    ...
    leg->SetTextSize(0.025);
    {   std::stringstream ss;
        ss << "16% quantile: " << setprecision(3) << quant16;
        leg->AddEntry((TObject*)0,ss.str().c_str(),"");
    }
    {   std::stringstream ss;
        ss << "median : " << setprecision(3) << median;
        leg->AddEntry((TObject*)0,ss.str().c_str(),"");
    }
    {   std::stringstream ss;
        ss << "84% quantile: " << setprecision(3) << quant84;
        leg->AddEntry((TObject*)0,ss.str().c_str(),"");
    }
    h1D->Draw("h");
    leg->Draw("same");
...
=======
    ...
    DrawPlot2D(pt,c1,h,flag,x,y,"signal",        "Signal Event Yield", -999, -999, s );
    DrawPlot2D(pt,c1,h,flag,x,y,"Acceptance",        "Acceptance", s->MinAccZ, s->MaxAccZ, s );
    DrawPlot2D(pt,c1,h,flag,x,y,"AcceptancePercent", "Acceptance [%]", -999, -999, s);
    DrawPlot2D(pt,c1,h,flag,x,y,"AcceptanceCorrected",    "Acceptance corr. f. sig. cont. [%]", s->MinAccZ*100, s->MaxAccZ*100, s );
    DrawPlot2D(pt,c1,h,flag,x,y,"ContaminationRelToSignal", "Signal contamination / Signal yield [%]", -999, -999, s );
