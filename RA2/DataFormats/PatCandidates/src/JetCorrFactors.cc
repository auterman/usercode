//
// $Id: JetCorrFactors.cc,v 1.10 2010/11/04 16:17:50 auterman Exp $
//
...
JetCorrFactors::print() const
{
  edm::LogInfo( "JetCorrFactors" )
    ...
    << "  +-L5(uds)    : " << std::setw(10) << uncertainty(L5uds,"up" ) << "(up) " << std::setw(10) << uncertainty(L5uds,  "down" ) << "(down)" << "\n"
    << "  +-L6(uds)    : " << std::setw(10) << uncertainty(L6uds,"up" ) << "(up) " << std::setw(10) << uncertainty(L6uds,  "down" ) << "(down)" << "\n"
    << "  +-L7(uds)    : " << std::setw(10) << uncertainty(L7uds,"up" ) << "(up) " << std::setw(10) << uncertainty(L7uds,  "down" ) << "(down)" 
    << std::endl;
}
...
JetCorrFactors::uncertDirection(const std::string& dir)
{
  ...
  if (dir=="UP" || dir=="up" || dir=="plus" || dir=="PLUS") 
    result=up;
  else if (dir=="DOWN" || dir=="DN"|| dir=="down" || dir=="minus" || dir=="MINUS")   
    result=down;
  else
    throw cms::Exception("InvalidRequest") 
...
{  
   std::string level = corrStep(step);
   std::string flav  = flavour(step);
   unsigned f = 0;
   if      (flav=="c") f=1;
   else if (flav=="b") f=2;
   else if (flav=="g") f=3;
   ...
   unsigned idx=0;
   if      (level=="off") idx=0;   //l1
   else if (level=="rel") idx=1;   //l2
   else if (level=="abs") idx=2;   //l3
   else if (level=="emf") idx=3;   //l4
   else if (level=="had") idx=4+f; //l5
   else if (level=="ue" ) idx=8+f; //l6
   else if (level=="part")idx=12+f;//l7
   else return 0;
   idx = 2*idx + direction;
   return correctionUncertainties_.at( idx );
}
