// @(#)root/hist:$Name:  $:$Id: TLimitDataSource.cc,v 1.2 2010/05/03 16:51:17 auterman Exp $
...
void TLimitDataSource::AddChannel(TH1D * s, TH1D * b, TH1D * d, 
				  TH1D * espos, TH1D * esneg,
                                  TH1D * ebpos, TH1D * ebneg, TObjArray * names)
{
   // Adds a channel with signal, background and data given as input.
   // In addition, error sources with asymmetric errors are defined.
   // TH1D are here used for convenience: each bin has to be seen as 
   // an error source (relative).
   // names is an array of strings containing the names of the sources.
   // Sources with the same name are correlated.
   
   fErrorOnSignalNeg.AddLast(esneg);
   fErrorOnBackgroundNeg.AddLast(ebneg);
   AddChannel(s,b,d,espos,ebpos,names);
}
...
void TLimitDataSource::SetOwner(bool swtch)
{
   ...
   fErrorOnSignalNeg.SetOwner(swtch);
   fErrorOnBackground.SetOwner(swtch);
   fErrorOnBackgroundNeg.SetOwner(swtch);
   fIds.SetOwner(swtch);
