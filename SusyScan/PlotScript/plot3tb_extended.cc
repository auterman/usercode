...
/**/
   ...
   TGraph * gCLsObsExclHT    = plotToolsHT->GetContour(hs,Mzero,Mhalf,NLOObsExclCL,	  3,1, 1,1); 
...
//   plotToolsHT->Print(Xsection, Mzero,Mhalf,gCLsObsExclHT, 10);
   ...
   TGraph * gCLsExpNoSExclMHT   = plotTools  ->GetContour(hs,Mzero,Mhalf,NLOExpNoSigExclCL,  3,1, 1,2); 
   TGraph * gCLsExpNoSExclHT    = plotToolsHT->GetContour(hs,Mzero,Mhalf,NLOExpNoSigExclCL,  3,1, 1,2); 
   ...
   TEV_sg_d0->Draw("fsame");
   TEV_sg_d0->Draw("lsame");
   ms.DrawLatex(1500,458,"tan#beta=3, #mu>0, A_{0}=0"); 
   ...
   sqt.DrawLatex(300,365,"#font[92]{#tilde{q}(800)GeV}");
   glt.DrawLatex(430,184,"#font[92]{#tilde{g}(500)GeV}");
   glt.DrawLatex(750,290,"#font[92]{#tilde{g}(800)GeV}");
   for(int i = 0; i < 4; i++){
    lngl[i]->Draw("same");   
    lnsq[i]->Draw("same");
   }
