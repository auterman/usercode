// $Id: Table.h,v 1.00 2013/05/26 20:00:00 auterman Exp $
...
namespace Table {
  ...
  enum TableLayout{Default, Transpose};
  ...
  class TTable {
   public:
    ...
    ///Used for printing:
    std::ostream& Get(std::ostream& os, unsigned x, unsigned y) const {os<<std::setw(GetWidth(x))
    									<<table_.at(x)->Str(y); return os;}
    ...
    void SetStyle(const TableStyle s){style_=s;};
    TableStyle GetStyle() const {return style_;};
    void SetLayout(const TableLayout s){layout_=s;};
    TableLayout GetLayout() const {return layout_;};
    ...
    unsigned GetWidth() const {return table_.at(idx_)->GetCurrentWidth();}
    unsigned GetWidth(unsigned idx) const {return table_.at(idx)->GetCurrentWidth();}
    unsigned GetNColumns() const {return (layout_==Default?table_.size():
                                   (table_.size()?table_.front()->Size():0));}
    unsigned GetNRows() const {return (layout_==Default?(table_.size()?table_.front()->Size():0):
                                   table_.size()); }
