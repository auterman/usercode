...
void drawCmsPrel(double intLumi, double METCut, bool onlyChannelInfo, int noJets, bool isBestjet) {
	...
	if (!onlyChannelInfo) {
		...
		as.DrawLatex(0.32, 0.93, Form(out.c_str(), intLumi/1000.));
	}
	...
	if (METCut == 0) {
		...
		out = "1#gamma, >=" + getStringFromInt(noJets) + " jets, MET>" + getStringFromInt(METCut)
			+ " GeV ";
		if (isBestjet)
			out = "1#gamma, >=2/3 jets, MET>" + getStringFromInt(METCut) + " GeV ";
	}
	as.SetTextSize(0.035);
	as.DrawLatex(0.57, 0.93, Form(out.c_str()));
	if (!onlyChannelInfo) {
		as.SetTextSize(0.04);
