 ...
 *
 * file: inputWinter11/data_Full2011.txt
 * * contains data event yields as used for the approval & ullas thesis (V20bt/bv/bw)
 * * but lumi is scaled to 4.618/fb for FSR/ISR AND FSR/ISR uncert. is lowered to 50%
 *
	...
	void WriteSingleBin(const std::string dir) {
		...
		for (vector<point>::iterator it = p_.begin(); it != p_.end(); ++it) {
		   //std::cout << "point " <<it-p_.begin()<<", bins = "<<it->bins.size() << std::endl;
			...
			double unc=sqrt( it->bins[bin].data +
      			            pow(it->bins[bin].u_scaleDataMC-1.,2) +
      			            pow(it->bins[bin].u_jes-1.,2) +
      			            pow(it->bins[bin].u_pdfacc-1.,2) +
				    pow(it->bins[bin].u_lumi-1,2) +
				    pow(it->bins[bin].u_qcd-1,2) +
				    pow(it->bins[bin].u_ewk-1,2) +
				    pow(it->bins[bin].u_fsr-1,2) +
				    pow(it->bins[bin].u_sig_stat-1,2) +
				    pow(it->bins[bin].u_qcd_stat-1,2) +
				    pow(it->bins[bin].u_ewk_stat-1,2) +
				    pow(it->bins[bin].u_fsr_stat-1,2) 
				  );
			double R=2.*unc/it->bins[bin].signal;
	  	 ...
	  	 if (a && !a->u_NLO){
	  		...
	  		//	   std::cout << "gl" <<a->gluino << std::endl;
	  	 	    // a->xsec        = p.xsec;
	  	 	     ...
	  	 	     a->u_NLO       = p.u_NLO - 1.0;
	  	 	     a->u_NLO_Up    = p.u_NLO_Up - 1.0;
	  	 	     a->u_NLO_Dn    = p.u_NLO_Dn - 1.0;
...
void Add_Xsec(std::string filelist) {
	...
	while (1) {
	   ...
	   if (a && !a->u_NLO){
	     ...
	     a->u_NLO_Dn    = -1.* fabs( NLO_dn / p.xsecNLO );
...
/*
   ...
   {
   Points.Reset();
   ReadSignalAcceptance("","inputWinter11/signalAcceptanceWino_preARC20120208.dat", "inputWinter11/data_Full2011.txt");
...
/*
   ...
   //3-jets
   Points.Reset();
   ReadSignalAcceptance("","inputWinter11/signalAcceptanceBinoNeutr_preARC20120208.dat", "inputWinter11/data_Full2011.txt");
   ...
   //2-jets
   Points.Reset();
   ReadSignalAcceptance("2j","inputWinter11/signalAcceptanceBinoNeutr_preARC20120208.dat", "inputWinter11/data_Full2011.txt");
   AddXsec("inputWinter11/binochixsec2_Dec1.dat");
   AddPDFxsec("inputWinter11/PDFcrossBino_NeutrScan.txt");
   AddPDFAcceptance("inputWinter11/PDFacceptanceBino_NeutrScan.txt");
   {points MergedPoints;
