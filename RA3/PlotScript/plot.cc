...
int plot(int argc, char** argv) {
	...
	PlotTools < SusyScan > *plotToolsMHT = new PlotTools<SusyScan> (
			genpoints->GetScan());
	...
	TH2F*hxsec = new TH2F("xsec",
			...
			2000, 20, 400, 2000);
	plotToolsMHT->Area(hxsec, Msquark, Mgluino, Xsection);
	...
	TH2F*hsigacc = new TH2F("sigacc",
			...
			2000, 20, 400, 2000);
	plotToolsMHT->Area(hsigacc, Msquark, Mgluino, SignalAcceptance);
	...
	plotToolsMHT->Area(TestCont, Msquark, Mgluino, ExpExclusionM1);
	/// try "3" contour levels:
	std::vector<TGraph*> contours = plotToolsMHT->GetContours(TestCont, 3);
	/// :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
	TestCont->Draw("colz");
	int col = kBlue - 10;
	...
	Smooth(gCLsObsExclMHT_LO_Smooth, 4);
	Smooth(gCLsExpExclMHT_LO_Smooth, 4);

	Smooth(gCLsExpExclMHTm1_Smooth, 4);
	Smooth(gCLsExpExclMHTp1_Smooth, 4);

	TGraph * gCLsExp1Sigma = MakeBand(gCLsExpExclMHTm1,gCLsExpExclMHTp1);
...
//	gCLsExpExclMHTp1->Draw("l");
	...
	leg->SetTextFont(42);
	TGraph * expLeg = (TGraph*) gCLsExpExclMHT_LO->Clone();
	...
	leg->AddEntry(gCLsExpExclMHTm1_Smooth, "Expected #pm 1#sigma, LO", "l");
	leg->AddEntry(gCLsExpExclMHT_LO_Smooth, "Expected, LO", "l");
