...
string file_T1lgScan = "2012-05-10-19-10-GMSB_T1lg2j";
string file_T1ggScan = "2012-05-10-18-52-GMSB_T1gg2j";

string file_wino3j = "2012-05-13-18-30-GMSBWino375Neutr";
string file_wino2j = "2012-05-09-22-24-GMSBWino375Neutr2j";
...
string file_bino3j = "2012-05-13-18-29-GMSBBino375Neutr";
string file_bino2j = "2012-05-09-21-44-GMSBBino375Neutr2j";
//the following configuration is automatically compared with result above
//configure legend titles, etc in 'setStyles'-function below
string file_bino2jSYST = "2012-02-08-17-37-GMSBBino375Neutr2j";

string file_binoNeutr3j = "2012-05-13-18-29-GMSB_SquarkGluino_vs_Neutralino";
string file_binoNeutr2j = "2012-05-09-22-33-GMSB_SquarkGluino_vs_Neutralino2j";
//the following configuration is automatically compared with result above
//configure legend titles, etc in 'setStyles'-function below
...
string file_bino2j_SingleBins = "2012-05-11-21-38-GMSBBino375NeutrSingleChannels2j";
string file_wino2j_SingleBins = "2012-05-11-21-38-GMSBWino375NeutrSingleChannels2j";
string file_binoNeutr2j_SingleBins = "2012-05-11-21-38-GMSB_SquarkGluino_vs_NeutralinoSingleChannels2j";

//string file_bino3j_DemoPoint = "2011-12-19-21-53-DemoPoint";
string file_bino2j_DemoPoint = "2012-05-21-18-37-DemoPointBino375Neutr2j";
...
void setStyles(PlotTools<SusyScan>*& PlotTool) {
	...
	retT1lg2j = retWino2j;
	retT1lg2j.flag = "T1lg";
	retT1lg2j.neutralinotype = "T1lg";
	retT1lg2j.drawGluinoNLSPExclusionRegion = true;

	retT1gg2j = retT1lg2j;
	retT1gg2j.flag = "T1gg";
	retT1gg2j.neutralinotype = "T1gg";
...
void CreateSamplePointTexFile(PlotTools<SusyScan> *PlotTool, double(*x)(const T*), double(*y)(const T*), double xValue, double yValue, string filename, string name, bool onlyAsympAvailable = false) {
	...
	if (signalacceptance >= 10)
		signalacceptance = PlotTool->roundDouble(signalacceptance, 0);
...
template<class T>
void CreateResultPointTexFile(TGraph * limit, PlotTools<SusyScan> *PlotTool, double(*x)(const T*), double(*y)(const T*), double xValue, string name, string filename,bool isNeutralinoScan) {
	...
	ofile << "\\def\\res" << name << "obsXSLimit{" << PlotTool->roundDouble(PlotTool->SingleValue(ObsXsecLimit, x, y, xValue, yValue), 3) << "}" << endl;
	...
	std::pair<double, double> minLimits = PlotTool->MinLimitValues(x, y, limit,isNeutralinoScan);
	std::pair<double, double> maxLimits = PlotTool->MaxLimitValues(x, y, limit,isNeutralinoScan);
	...
	ofile << "\\def\\res" << name << "maxY{" << maxLimits.second << "}" << endl;
	ofile.close();
}
...
void PlotAllObserved(std::vector<ExclusionCurves*> limits, PlotStyles * style, TH1*h) {
	...
	h->UseCurrentStyle();
	{
		...
		if (neutralinotype == ("T1lg"))
			legtitle = "#splitline{SMS #gamma+X}{#slash{E}_{T} [GeV]:}";
		if (neutralinotype == ("T1gg"))
			legtitle = "#splitline{SMS #gamma#gamma+X}{#slash{E}_{T} [GeV]:}";
		TLegend * leg = new TLegend(0.62, 0.54, 0.95, 0.87, "");
		if (style->drawGluinoNLSPExclusionRegion) {
			leg = new TLegend(0.22, 0.54, 0.55, 0.87, "");
		}
		leg->SetBorderSize(0);
		leg->SetLineColor(0);
		leg->SetFillColor(10);
	...
	}
	{
		...
		if (style->drawGluinoNLSPExclusionRegion) {
					leg = new TLegend(0.22, 0.52, 0.55, 0.87, "");
				}
		leg->SetBorderSize(0);
		leg->SetLineColor(0);
		leg->SetFillColor(10);
...
void DrawExclusion(ExclusionCurves * exclA, PlotStyles * style, TH1*h, ExclusionCurves * exclB = 0, bool plotAllErrors = false) {
	...
	{//Exclusion Contours
		...
		if (neutralinotype == ("T1lg"))
			legtitle = "SMS #gamma+X";
		if (neutralinotype == ("T1gg"))
			legtitle = "SMS #gamma#gamma+X";

		TLegend * leg = new TLegend(0.68, 0.58, 0.95, 0.87);
		if ((neutralinotype == ("T1lg"))||(neutralinotype == ("T1gg"))) {
					leg = new TLegend(0.38, 0.58, 0.65, 0.87, "");
				}
		if (drawComp) {
			leg = new TLegend(0.68, 0.5, 0.95, 0.87);
		}
		...
		if (neutralinotype != ("T1lg")&&neutralinotype != ("T1gg"))leg->AddEntry((TObject*) 0, "", "");
...
//			lat3->Draw();
		...
		if (!drawComp)
			drawCMSPrel = true;
		drawCmsPrel(style->lumi, style->METCut, style->noJet, style->isBestJet, "", true, drawCMSPrel);
...
void interpolateAndFillPoints(TheLimits * genpointsInterpol, TheLimits * genpoints, bool isNeutrGluinoScan = false, bool interpolate=true) {
...
void GetPlotTools(PlotTools<SusyScan>*& plotTools, PlotTools<SusyScan>*& plotToolsInterpol, std::string filename, bool isNeutrGluinoScan = false, bool isNeutrSquarkScan = false, bool interpolate=true, string fileappendix = "/filelist.txt") {
	...
	//cout << "interpol..." << endl;
	interpolateAndFillPoints(genpointsInterpol, genpoints, isNeutrGluinoScan,interpolate);
...
void PlotBinComparison(PlotTools<SusyScan>*& Scan, PlotTools<SusyScan>*& ScanInterpol, double(*x)(const T*), double(*y)(const T*), string filename, string filenameSingleBins, PlotStyles ret, TH1 * hi, TH1 * h, bool excludeBelowExcludedRegion = false, string filenameforresulttex = "") {
	...
	GetPlotTools(Scan, ScanInterpol, filenameSingleBins, excludeBelowExcludedRegion, false,true, "/filelist_ch0.txt");
	...
	GetPlotTools(Scan, ScanInterpol, filenameSingleBins, excludeBelowExcludedRegion, false,true, "/filelist_ch1.txt");
	...
	GetPlotTools(Scan, ScanInterpol, filenameSingleBins, excludeBelowExcludedRegion, false,true, "/filelist_ch2.txt");
	...
	GetPlotTools(Scan, ScanInterpol, filenameSingleBins, excludeBelowExcludedRegion, false,true, "/filelist_ch3.txt");
	...
	GetPlotTools(Scan, ScanInterpol, filenameSingleBins, excludeBelowExcludedRegion, false,true, "/filelist_ch4.txt");
	...
	GetPlotTools(Scan, ScanInterpol, filenameSingleBins, excludeBelowExcludedRegion, false,true, "/filelist_ch5.txt");
...
int plot(int argc, char** argv) {
	...
	//TH2F hNeutrGluinoSMSi("hNeutrGluinoSMSi", ";m_{#chi^{0}_{1}} [GeV]; m_{#tilde{g}} [GeV]; cross section [pb]", 143, 96.875, 1878.125,129, 96.875, 2003.125);

	PlotTools<SusyScan> *Scan, *ScanInterpol;

	///==================T1lg Neutr vs Gluino Limits 2j================================
	GetPlotTools(Scan, ScanInterpol, filenames::file_T1lgScan, true,false,false);
	setStyles(Scan);
	...
	GetPlotTools(Scan, ScanInterpol, filenames::file_T1ggScan, true,false,false);
	setStyles(Scan);
	...
	//
		...
		GetPlotTools(Scan, ScanInterpol, filenames::file_bino2j);
		CreateResultPointTexFile(exclBino2j.p1sigmaNLOXSObs, ScanInterpol, Msquark, Mgluino, 1300, "Bino", filenameforresulttex,false);
		GetPlotTools(Scan, ScanInterpol, filenames::file_wino2j);
		CreateResultPointTexFile(exclWino2j.p1sigmaNLOXSObs, ScanInterpol, Msquark, Mgluino, 1000, "Wino", filenameforresulttex,false);
		GetPlotTools(Scan, ScanInterpol, filenames::file_binoNeutr2j);
		CreateResultPointTexFile(exclBinoNeutr_gl_2j.p1sigmaNLOXSObs, ScanInterpol, Mchi1, Mgluino, 150, "BinoNeutrMin", filenameforresulttex,true);
		CreateResultPointTexFile(exclBinoNeutr_gl_2j.p1sigmaNLOXSObs, ScanInterpol, Mchi1, Mgluino, 900, "BinoNeutrMax", filenameforresulttex,true);
