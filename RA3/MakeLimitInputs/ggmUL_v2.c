...
static Double_t MinLike = 1.e-7, Precision = 1.e-5;
static Int_t N;
...
// SashaL: upper limit for integration in terms of sigmas
static Double_t nSig = 5.0;
...
double kF[nGrid][nGrid];
double xSecErrLo[nGrid][nGrid];
double xSecErrHi[nGrid][nGrid];
double accValue[nGrid][nGrid];
double accErrStat[nGrid][nGrid];
double xSecValue[nGrid][nGrid];
...
// (N is a global parameter. The parameter p is dummy).
{
	...
	if (sB == 0.)
	{
		if (sA == 0.) 
		{
		        // In this case, there is zero uncertainty on
		        // the nuisance parameters, so the likelihood
		        // is simply a Poission (or Gaussian).
			if (lGauss) retval = TMath::Gaus(N-B0-x[0]*A0,0.,TMath::Sqrt(N),kTRUE);
			else retval = Poisson(B0+x[0]*A0,N);
		}
		else 
		{
		        // In this case, the parameter A0 (lumi*effi)
		        // is uncertain, so integrate the simple
		        // Poission over all possible values of A0,
		        // weighted by apriori prob. that that
		        // parameter is correct.

			Double_t par[2];
			//		  TF1 *in = new TF1("Inner",Inner,0.,TMath::Max(A0 + 5.*sA,2);
			TF1 *in = new TF1("Inner",Inner,0.,A0 + nSig*sA,2);
			par[0]=B0;  // background value
			par[1]=x[0];  // signal cross section value
			if (I == 0)
			{
				Double_t low = A0 > nSig*sA ? A0 - nSig*sA : 0.;
				retval = in->Integral(low,A0+nSig*sA,par,epsilon);
			} else {
				Double_t low = 0., high = A0 + nSig*sA, tmp = 1.;
				while (tmp > epsilon)
				{	 
	...
	else
	{
		...
		TF1 *out = new TF1("Outer",Outer,0.,B0 + nSig*sB,1);   
		if (I == 0)
		{
			Double_t low = B0 > nSig*sB ? B0 - nSig*sB : 0.;
			retval = out->Integral(low,B0+nSig*sB,x,epsilon);
		} else {
			Double_t low = 0., high = B0 + nSig*sB, tmp = 1.;
			while (tmp > epsilon)
			{	 
	...
	return retval;  // flat prior
	//	return retval/sqrt(x[0]); // Jeffreys' prior for Poisson
}
...
// This function can make use of function Inner, which also takes into account uncertainty in lumi*effi.
{
	...
	else 
	{
		...
		Double_t low = A0 > nSig*sA ? A0 - nSig*sA : 0.;
		if (I == 0) retval = TMath::Gaus(x[0],B0,sB,kTRUE)*in->Integral(low,A0+nSig*sA,par,epsilon);
		else if (I == 1) retval = TMath::LogNormal(x[0], sigma_b, 0., B0)*in->Integral(0.,A0+nSig*sA,par,epsilon);
		else if (I == 2) retval = TMath::GammaDist(x[0], sigma_b, 0., 1./tau_b)*in->Integral(0.,A0+nSig*sA,par,epsilon);
		else return 0;
		delete in;
	}
	return retval;
...
double myGausT( double *x, double *par)
{
  double f = 0.;
  if( x[0]<=0.) return f;
  if( par[0]<=0.) return f;
  if( par[1]<=0.) return f;
  return TMath::Gaus(x[0], par[0], par[1], false)/par[2];
}
...
void PlotGausT(double mean, double sigma, double integral)
{		
  if( mean <=0 ) return;
  TF1 *in = new TF1("myGausT",myGausT,0.,mean+100.0*sigma,3);
  double x[2];
  double parG[3];
  parG[0] = mean;
  parG[1] = sigma;
  ...
  for(int ip=0; ip!=1000; ip++){
    x[0] = 0.0 + 0.0001*(ip+0.5);
    double yTmp = myGausT(&x[0], &parG[0]);
    grA->SetPoint(ip, x[0], yTmp);
  }
  grA->Draw("AL");
  double eps = 1.e-7;
  for(int is=1; is!=50; is++){
    cout << is << " " <<  in->Integral(0.,mean+is*sigma,parG,eps) << endl;
  }
  delete in;
...
double myLogNormal( double *x, double *par)
{
  double f = 0.;
  if( x[0]<=0.) return f;
  if( par[0]<=0.) return f;
  if( par[1]<=0.) return f;
  double s = TMath::Log(1. + par[1]/par[0]);
  return TMath::LogNormal(x[0], s, 0., par[0]);
}


void PlotLogNormal(double mean, double sigma)
{		
  if( mean <=0 ) return;
  TF1 *in = new TF1("myLogNormal",myLogNormal,0.,mean+100.0*sigma,2);
  double x[2];
  double parG[2];
  parG[0] = mean;
  parG[1] = sigma;
  TGraph *grA = new TGraph();
  double step = 0.001*(mean+10.0*sigma);
  for(int ip=0; ip!=1000; ip++){
    x[0] = 0.0 + step*(ip+0.5);
    double yTmp = myLogNormal(&x[0], &parG[0]);
    grA->SetPoint(ip, x[0], yTmp);
  }
  grA->Draw("AL");
  double eps = 1.e-7;
...
double myGamma( double *x, double *par)
{
  ...
  if( par[0]<=0.) return f;
  if( par[1]<=0.) return f;
  double s = 1. + par[0]*par[0]/par[1]/par[1];
  double t = par[0]/par[1]/par[1];
  return TMath::GammaDist(x[0], s, 0., 1./t);
}


void PlotGamma(double mean, double sigma)
{		
  if( mean <=0 ) return;
  if( sigma <=0 ) return;
  TF1 *in = new TF1("myGamma",myGamma,0.,mean+100.0*sigma,2);
  double x[2];
  double parG[2];
  parG[0] = mean;
  parG[1] = sigma;
  ...
  for(int ip=0; ip!=1000; ip++){
    x[0] = 0.0 + 0.01*(ip+0.5);
    double yTmp = myGamma(&x[0], &parG[0]);
    grA->SetPoint(ip, x[0], yTmp);
  }
  grA->Draw("AL");
  double eps = 1.e-7;
  for(int is=3; is!=50; is++){
    cout << is << " " <<  in->Integral(0.,mean+is*sigma,parG,eps) << endl;
  }
  delete in;
}
...
void SaveGrid(char *filenameOut)
{

  TFile *fout = new TFile(filenameOut,"recreate");
  TTree *myTree = new TTree("T"," ");
  double mGluino;
  double mSquark;
  double mNeutralino;
  int nGen;
  ...
  double limit;
  double explimit;
  myTree->Branch("mGluino",&mGluino,"mGluino/D");
  myTree->Branch("mSquark",&mSquark,"mSquark/D");
  myTree->Branch("mNeutralino",&mNeutralino,"mNeutralino/D");
  myTree->Branch("nGen",&nGen,"nGen/I");
  myTree->Branch("nAcc",&nAcc,"nAcc/I");
  myTree->Branch("accValue",&accValue,"accValue/D");
  myTree->Branch("accErrorStat",&accErrorStat,"accErrorStat/D");
  myTree->Branch("accErrorSyst",&accErrorSyst,"accErrorSyst/D");
  myTree->Branch("accErrorPDF",&accErrorPDF,"accErrorPDF/D");
  myTree->Branch("accErrorTot",&accErrorTot,"accErrorTot/D");
  myTree->Branch("kfactor",&kfactor,"kfactor/D");
  myTree->Branch("xsecValue",&xsecValue,"xsecValue/D");
  myTree->Branch("xsecErrorNeg",&xsecErrorNeg,"xsecErrorNeg/D");
  myTree->Branch("xsecErrorPos",&xsecErrorPos,"xsecErrorPos/D");
  myTree->Branch("limit",&limit,"limit/D");
  myTree->Branch("explimit",&explimit,"explimit/D");
  for(unsigned int i=0; i<grid.size(); i++)
    {
      mGluino = grid[i].mGluino; 
      mSquark = grid[i].mSquark;
      mNeutralino = grid[i].mNeutralino;
      nGen = grid[i].nGen;
      nAcc = grid[i].nAcc;
      accValue = grid[i].accValue;
      accErrorStat = grid[i].accErrorStat;
      accErrorSyst = grid[i].accErrorSyst;
      accErrorPDF = grid[i].accErrorPDF;
      accErrorTot = grid[i].accErrorTot;
      kfactor = grid[i].kfactor;
      xsecValue = grid[i].xsecValue;
      xsecErrorNeg = grid[i].xsecErrorNeg;
      xsecErrorPos = grid[i].xsecErrorPos;
      limit = grid[i].limit;
      explimit = grid[i].explimit;
      myTree->Fill();
    }
...
void CreateGrid()
{
  grid.clear();
  ifstream fin;
  fin.open("MC_Map_Dec5.dat");
  while(1){
    int mG, mS, nGen, nAcc;
    double mChi, xsec;
    fin >> mG >> mS >> mChi >> nGen >> nAcc >> xsec;
    if(!fin.good()) break;
    GridPoint p;
    p.mGluino        = mG;
    p.mSquark        = mS;
  ...
  fin.close();
  cout << " Loaded " << grid.size() << " grid points " << endl;
}
...
{
  ifstream fin;
  fin.open("ProspinoKfactorsDiphotonsAll.txt");
  while(1){
    int mG, mS;
    double kfactor;
    fin >> mG >> mS >> kfactor;
    ...
    for(unsigned int ig=0; ig<grid.size(); ig++){
      if( fabs(mG-grid[ig].mGluino)<0.5 
	  && fabs(mS-grid[ig].mSquark)<0.5
	  ) grid[ig].kfactor=kfactor;
    }
  }
  fin.close();
}
...
void GetPDFErrorsXSection()
{
  ifstream fin;
  fin.open("xsectionPDFErrors.dat");
  while(1){
    int mG, mS;
    double xs;
    fin >> mG >> mS >> xs;
	...
	{
	  grid[ig].xsecErrorNeg = 0.01*xs;
	  grid[ig].xsecErrorPos = 0.01*xs;
	}
    } 
  }
  fin.close();
...
void GetPDFErrorsAcceptance()
{
  ifstream fin;
  fin.open("acceptancePDFErrors.dat");
  while(1){
    int mG, mS;
    double xs;
    fin >> mG >> mS >> xs;
...
void InterpolatePotHoles(TH2D *hist, int flag)
{
  double q11=0;
  double q12=0;
  double q21=0;
  ...
  case 1:
    ...
    q11 = hist->GetBinContent(6,5);
    q22 = hist->GetBinContent(9,8);
    f = q11 + (q22-q11)/3.;
    hist->SetBinContent(7,6,f);
    f = q11 + (q22-q11)/3.;
    hist->SetBinContent(7,6,f);
    f = q11 + 2.*(q22-q11)/3.;
    hist->SetBinContent(8,7,f);

    q11 = hist->GetBinContent(12,11);
    q12 = hist->GetBinContent(14,11);
    q21 = hist->GetBinContent(13,10);
    q22 = hist->GetBinContent(13,12);
    f = (q11+q12+q21+q22)/4.;
    hist->SetBinContent(13,11,f);

    q11 = hist->GetBinContent(14,11);
    q12 = hist->GetBinContent(16,11);
    q21 = hist->GetBinContent(15,10);
    q22 = hist->GetBinContent(15,12);
    f = (q11+q12+q21+q22)/4.;
    hist->SetBinContent(15,11,f);

    q11 = hist->GetBinContent(15,14);
    q12 = hist->GetBinContent(15,12);
    f = (q11+q12)/2.;
    hist->SetBinContent(15,13,f);

    q11 = hist->GetBinContent(18,15);
    q12 = hist->GetBinContent(18,13);
    f = (q11+q12)/2.;
    hist->SetBinContent(18,14,f);

    q11 = hist->GetBinContent(17,15);
    q12 = hist->GetBinContent(17,13);
    f = (q11+q12)/2.;
    hist->SetBinContent(17,14,f);

    q11 = hist->GetBinContent(15,13);
    q12 = hist->GetBinContent(17,13);
    f = (q11+q12)/2.;
    hist->SetBinContent(16,13,f);

    q11 = hist->GetBinContent(15,14);
    q12 = hist->GetBinContent(17,14);
    q21 = hist->GetBinContent(16,15);
    q22 = hist->GetBinContent(16,13);
    f = (q11+q12+q21+q22)/4.;
    hist->SetBinContent(16,14,f);

    q11 = hist->GetBinContent(15,6);
    q12 = hist->GetBinContent(17,6);
    f = (q11+q12)/2.;
    hist->SetBinContent(16,6,f);

    q11 = hist->GetBinContent(17,6);
    q12 = hist->GetBinContent(17,4);
    f = (q11+q12)/2.;
    hist->SetBinContent(17,5,f);

    q11 = hist->GetBinContent(15,5);
    q12 = hist->GetBinContent(17,5);
    q21 = hist->GetBinContent(16,6);
    q22 = hist->GetBinContent(16,4);
    f = (q11+q12+q21+q22)/4.;
    hist->SetBinContent(16,5,f);

    q11 = hist->GetBinContent(19,4);
    q12 = hist->GetBinContent(21,4);
    q21 = hist->GetBinContent(20,5);
    q22 = hist->GetBinContent(20,3);
    f = (q11+q12+q21+q22)/4.;
    hist->SetBinContent(20,4,f);

    q11 = hist->GetBinContent(21,12);
    q12 = hist->GetBinContent(21,10);
    f = (q11+q12)/2.;
    hist->SetBinContent(21,11,f);

    q11 = hist->GetBinContent(19,11);
    q12 = hist->GetBinContent(21,11);
    q21 = hist->GetBinContent(20,12);
    q22 = hist->GetBinContent(20,10);
    f = (q11+q12+q21+q22)/4.;
    hist->SetBinContent(20,11,f);

    break;

  case 2:

    q11 = hist->GetBinContent(4,18);
    q12 = hist->GetBinContent(6,18);
    q21 = hist->GetBinContent(5,19);
    q22 = hist->GetBinContent(5,17);
    f = (q11+q12+q21+q22)/4.;
    hist->SetBinContent(5,18,f);

    q11 = hist->GetBinContent(14,20);
    q12 = hist->GetBinContent(16,20);
    q21 = hist->GetBinContent(15,21);
    q22 = hist->GetBinContent(15,19);
    f = (q11+q12+q21+q22)/4.;
    hist->SetBinContent(15,20,f);

    q11 = hist->GetBinContent(13,10);
    q12 = hist->GetBinContent(15,10);
    q21 = hist->GetBinContent(14,11);
    q22 = hist->GetBinContent(14,9);
    f = (q11+q12+q21+q22)/4.;
    hist->SetBinContent(14,10,f);

    q11 = hist->GetBinContent(15,10);
    q12 = hist->GetBinContent(17,10);
    q21 = hist->GetBinContent(16,11);
    q22 = hist->GetBinContent(16,9);
    f = (q11+q12+q21+q22)/4.;
    hist->SetBinContent(16,10,f);

    q11 = hist->GetBinContent(17,9);
    q12 = hist->GetBinContent(19,9);
    f = (q11+q12)/2.;
    hist->SetBinContent(18,9,f);

    q11 = hist->GetBinContent(19,11);
    q12 = hist->GetBinContent(19,9);
    f = (q11+q12)/2.;
    hist->SetBinContent(19,10,f);

    q11 = hist->GetBinContent(17,10);
    q12 = hist->GetBinContent(19,10);
    q21 = hist->GetBinContent(18,11);
    q22 = hist->GetBinContent(18,9);
    f = (q11+q12+q21+q22)/4.;
    hist->SetBinContent(18,10,f);

    q11 = hist->GetBinContent(14,13);
    q12 = hist->GetBinContent(16,13);
    q21 = hist->GetBinContent(15,14);
    q22 = hist->GetBinContent(15,12);
    f = (q11+q12+q21+q22)/4.;
    hist->SetBinContent(15,13,f);

    q11 = hist->GetBinContent(21,18);
    q12 = hist->GetBinContent(21,16);
    f = (q11+q12)/2.;
    hist->SetBinContent(21,17,f);

    q11 = hist->GetBinContent(19,16);
    q12 = hist->GetBinContent(21,16);
    q21 = hist->GetBinContent(20,17);
    q22 = hist->GetBinContent(20,15);
    f = (q11+q12+q21+q22)/4.;
    hist->SetBinContent(20,16,f);

    break;

  case 3:

    q11 = hist->GetBinContent(13,9);
    q12 = hist->GetBinContent(15,9);
    f = (q11+q12)/2.;
    hist->SetBinContent(14,9,f);

    q11 = hist->GetBinContent(16,14);
    q12 = hist->GetBinContent(18,14);
    q21 = hist->GetBinContent(17,15);
    q22 = hist->GetBinContent(17,13);
    f = (q11+q12+q21+q22)/4.;
    hist->SetBinContent(17,14,f);

    q11 = hist->GetBinContent(18,12);
    q12 = hist->GetBinContent(20,12);
    q21 = hist->GetBinContent(19,13);
    q22 = hist->GetBinContent(19,11);
    f = (q11+q12+q21+q22)/4.;
    hist->SetBinContent(19,12,f);

    q11 = hist->GetBinContent(18,17);
    q12 = hist->GetBinContent(20,17);
    q21 = hist->GetBinContent(19,18);
    q22 = hist->GetBinContent(19,16);
    f = (q11+q12+q21+q22)/4.;
    hist->SetBinContent(19,17,f);
    ...
    q11 = hist->GetBinContent(14,13);
    q12 = hist->GetBinContent(16,15);
    f = (q11+q12)/2.;
    hist->SetBinContent(15,14,f);

    q12 = hist->GetBinContent(20,9);
    q21 = hist->GetBinContent(21,10);
    q22 = hist->GetBinContent(20,10);
    f =  (q12+q21) - q22;
    hist->SetBinContent(21,9,f);
...
void CalculateAcceptanceCorrectedJan2011()
{
  ...
  double effCorrValue = 0.967;

  // relative error for single photon Data/MC correction
  // it is a sum of
  // 1.1% error 0.967
  // 1% due to pileup
  // 0.5% due to photon/electron difference
  double effCorrError = sqrt( 0.011*0.011 + 0.01*0.01 + 0.005*0.005 );
...
void Start()
{
  CreateGrid();
  GetKFactors();
  GetPDFErrorsXSection();
  GetPDFErrorsAcceptance();
  CalculateAcceptanceCorrectedJan2011();

  unsigned int nUL = grid.size();

  // ------------ our measurement here ----------------
  int n = 1;                      // candidates
  double ilum = 35.48;            // int. luminosity
  double slum = 0.11*ilum;        // 11% on lumi
  double bck  = 1.2;             // background level (0.53+-0.40), (1.71+-0.66), (1.2+-0.8)
  double sbck = 0.8;             // error on background
  ...
  for(unsigned int i=0; i<nUL; i++)
    {
      cout << " calculating upper limit   " << i << "  out of " << nUL << endl;
      double eff  = grid[i].accValue;
      double seff = grid[i].accErrorTot;
      bool gauss = false;
      plot = false;
      grid[i].limit =  CL95(ilum, slum, eff, seff, bck, sbck, n, gauss, 1);
      grid[i].explimit =  CLA(ilum, slum, eff, seff, bck, sbck, 1);
    }

  PrintGrid();
  SaveGrid("output.root");
}
...
void TestSinglePoint()
{
  ...
  double mGluino_p = 720.0;
  double mSquark_p = 720.0;
  double mNeutralino_p = 150.0;
  for(unsigned int i=0; i<nUL; i++)
    {
      if( fabs(grid[i].mGluino-mGluino_p)<0.5
	  && fabs(grid[i].mSquark-mSquark_p)<0.5
	  && fabs(grid[i].mNeutralino-mNeutralino_p)<0.5
	  )
	{
	  double eff  = grid[i].accValue;
	  double seff = grid[i].accErrorTot;
	  bool gauss = false;
	  plot = false;
	  double limit0 =  CL95(ilum, slum, eff, seff, bck, sbck, n, gauss, 0);
	  double limit1 =  CL95(ilum, slum, eff, seff, bck, sbck, n, gauss, 1);
	  double limit2 =  CL95(ilum, slum, eff, seff, bck, sbck, n, gauss, 2);
	  cout << limit0 << endl;
	  cout << limit1 << endl;
	  cout << limit2 << endl;
	  // double avglimit =  CLA(ilum, slum, eff, seff, bck, sbck, 1);
	}
    }
